...
class Track():
    ...
    def loadFromFile(self,filename):
        with open(filename) as f:
            ...
            #get map image filename
            self.mapfile = lines[1].strip()
            #get map resolution
            self.res = [int(dim) for dim in lines[3].split()]
            #read track points
            self.point = []
            i=5
            while True:
                ...
                self.point.append(struct(x=int(coords[0]),y=int(coords[1]),width=10))
                i+=1
            #read cameras
            self.cams = []
            i=i+1
            for line in lines[i:]:
                if line == "": break 
                self.cams.append([int(coord) for coord in line.split()])
...
class Detector():
    ...
    def __init__(self,directory,cams,server):
        self.cams=cams
        self.names=[]
        self.server=server
    ...
    def detect(self,car):
        ...
        detections=[]
        results=[]
        for name, cam in zip(self.names,self.cams):
            #detect on camera
            detection=self.oncam((rx,ry),(cam[0],cam[1]),(cam[2],cam[3]),(cam[4],cam[5]))        
            #fuzzyfi inputs
            url = self.server+name+"/run/"
            cmd = name+'["CamX"] = '+name+'["CamX"].inputMF.evaluate('+str(rx)+')'
            self.request.post(url,data = {'command':cmd})
            cmd = name+'["CamY"] = '+name+'["CamY"].inputMF.evaluate('+str(ry)+')'
            self.request.post(url,data = {'command':cmd})
            cmd = name+'["Detection"] = '+name+'["Detection"].inputMF.evaluate('+str(detection)+')'
            self.request.post(url,data = {'command':cmd})
            #produce outputs
            cmd = name+'.update()'
            self.request.post(url,data = {'command':cmd})
            #get outputs
            cmd = name+'["OutputX"].outputMF.evaluate('+name+'["OutputX"].value)'
            dx = self.request.post(url,data = {'command':cmd})
            cmd = name+'["OutputY"].outputMF.evaluate('+name+'["OutputY"].value)'
            dy = self.request.post(url,data = {'command':cmd})
            cmd = name+'["Confidence"].outputMF.evaluate('+name+'["Confidence"].value)'
            dd = self.request.post(url,data = {'command':cmd})
            results.append((dx,dy,dd))
        for res in results:    
            x = float(res[0].result().text)
            y = float(res[1].result().text)
            d = float(res[2].result().text)
