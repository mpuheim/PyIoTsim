...
from modules.fcm import FCMcontroller
...
# interaction
interactivity = True
interact = 1 #generations
# stopping conditions
lap_time = 60 #seconds
# track
trackfile = "tracks/default.track"
# camera models
camfiles = "models/cams"
...
# init track
track = simulation.Track(trackfile)
...
for i in range(size):
    ...
    individual.fenotype = FCMcontroller(individual.genotype.chromosome)
...
for ind in population:
    # init car
    car = simulation.Car(x_pos=track.point[start].x, y_pos=track.point[start].y, direction=track.gate[start].angle-110)
    # init detector
    detector = simulation.Detector(camfiles,track.cams)
    # init navigator
...
while True:
    ...
    for i in range(migration):
        ...
        migrant.fenotype = FCMcontroller(migrant.genotype.chromosome)
    ...
    for i in range(offsprings):
        ...
        # generate fenotype
        child.fenotype = FCMcontroller(child.genotype.chromosome)
        # add to population
